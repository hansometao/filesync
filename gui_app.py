...
class TaskCard(ttk.Frame):
    ...
    def __init__(self, master, app, task_id):
        ...
        self._name_lbl = tk.Label(info, font=("", 11, "bold"),
        ...
        # 右键菜单绑定（整卡都响应）
        for w in (self, self._sw_canvas, info, self._name_lbl,
                  self._path_lbl, self._sched_lbl, right, self._next_lbl,
    ...
    def refresh(self, task):
        ...
        if task.schedule.enabled and task.enabled:
            from utils.timeutil import next_daily_times, next_weekly_times
    ...
    def set_selected(self, sel):
        ...
        if sel:
            ...
            for w in (self._name_lbl, self._path_lbl, self._sched_lbl,
                      self._next_lbl, self._status_lbl, self._sw_canvas):
                try:
...
class App(SyncFlowMixin, TrayMenuMixin, CloseSeqMixin):
    ...
    def _build_ui(self):
        # type: () -> None
        self.root.title("FileGo v%s — 定时文件同步工具" % APP_VERSION)
        ...
        tk.Label(brand_left, text="FileGo", font=("", 14, "bold"),
        ...
        self._task_canvas.create_window((0, 0), window=self._task_inner,
                                        anchor="nw")
    ...
    def _on_run_all(self):
        # type: () -> None
        """运行所有已启用的任务（串行，避免并发争用 CPU）。"""
        tasks = [t for t in self.store.snapshot()
                 if t.enabled and not self.scheduler.is_task_running(t.id)]
        if not tasks:
            messagebox.showinfo("提示", "没有可运行的任务")
            return

        # 串行手动运行：先跑一个等完成，再跑下一个
        def _run_seq(idx):
            if idx >= len(tasks) or self._closing:
                self._refresh_tasks(full=True)
                return
            t = tasks[idx]
            self.logger.info("批量运行[%d/%d]: %s" % (
                idx + 1, len(tasks), t.name))
            # 复用 SyncFlowMixin._on_sync_now 的完整流程
            self._selected_id = t.id
            if t.id in self._task_rows:
                self._task_rows[t.id].set_selected(True)
            # _on_sync_now 会在完成后自动调用 _refresh_tasks
            # 然后链式调用下一个
            old = getattr(self, "_on_sync_now", None)
            if old is None:
                return
        ...
        for t in tasks:
            if self._closing:
                break
            self._selected_id = t.id
            # 直接调用 perform_sync + finalize_sync，避免弹窗确认
            try:
                res = perform_sync(t, logger=self.logger,
                                   self_paths=self.self_paths)
                finalize_sync(t, res, self.store, self.logger)
            except Exception as e:
                self.logger.error("批量运行[%s] 异常: %s" % (t.name, e))
        self._refresh_tasks(full=True)
